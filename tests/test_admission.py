...
class AdmissionPlanTests(unittest.TestCase):
    ...
    def test_each_mutable_governed_plan_field_changes_the_canonical_digest(self) -> None:
        ...
        cases = {
            ...
            "areas_to_avoid": changed(lambda value: value["items"][0]["contract"].update(areas_to_avoid=[{"path": "floati/other.py", "region": "all"}])),
            "areas_to_avoid_region": changed(lambda value: value["items"][0]["contract"].update(areas_to_avoid=[{"path": "floati/graph.py", "region": "part"}])),
