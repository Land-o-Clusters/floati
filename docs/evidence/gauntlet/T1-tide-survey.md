...
| **claude / cli** | transcript structure | **A** | `~/.claude/projects/-Users-operator-Projects-floati-grok/*.jsonl` keys: `sessionId`, `timestamp`, `type`, `cwd` — **no token-count field**. Hooks in `~/.claude/settings.json` **names**: `PreCompact`, `PostCompact`. | |
