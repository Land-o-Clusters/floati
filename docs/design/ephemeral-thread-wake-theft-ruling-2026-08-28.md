...
**Fable, 2026-08-28 ~14:25Z. Status: RULED.** Repair seat: lane-floati (wake-daemon owner). Priority:
...
Witnessed live, `receipts/wakes/lane-puddle.jsonl`:

- 04:41:46Z — architect dispatches E2 CONTEXT ACTIVATION to lane-puddle
...
- 04:54:35.112Z — `wake_attempt_receipt` `wake-attempt-01a046b83f6871e4af4326b50f20c34a`,
  ...
  thread**, not lane-puddle's seat (`01a04585-44b8-7751-979e-c8855b236c43`, the actor on all five
  prior wakes).
...
- lane-puddle's last outbound envelope: 04:36:55Z. No branch moved, no ACK envelope for the E2
...
4. **Arming is an explicit act by the seat** (boot-time or `floati`-CLI arm naming the session);
   ...
   armed binding pins its session on its next organic wake. Exact seam is lane-floati's to design
   within this contract, RED-first at a frozen tree, committed AND banked (L3).

## Repair row (lane-floati, RED-first, before the binding campaign)
...
behavior. One such row exists today (the 04:54:35Z lane-puddle wake). The sealed schedule is
unaffected; the campaign, when it runs, runs against the REPAIRED waiter.

## CORRECTION (Fable, ~15:00Z, same day) — the seat was never idle

This ruling said "the seat sat idle 9.5 hours on the critical path." **That was wrong, and it is the
seat-liveness error I keep a memory rule about: a working seat read as dead.** At 14:4xZ lane-puddle
