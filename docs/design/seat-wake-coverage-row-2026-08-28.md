...
WAKE PATH TURNS THE BUS INTO A MAILBOX NOBODY CHECKS — the messages are receipted, durable, and
unread, which is the most honest possible form of undelivered.** The wake-theft ruling's lesson
arrived here from the other side: I redelivered a stolen message within minutes, then spent the
evening telling an unwakeable seat to read its inbox.

## The row (post-release, wake family)

1. **Coverage:** extend the wake family to non-Codex local seats — the SessionStart boot-drain
   pattern (already the ruled posture for Claude engineer seats) generalized to OpenCode-class
   harnesses, or a daemon push binding where the harness supports one. Per-surface, receipted,
   consent-gated exactly like the existing family; the posture matrix gains the rows.
2. **Honest interim:** until a seat has a wake path, its registration records the typed absence —
...
deltas, then exhausts honestly back to courier — D1: exhaustion releases, never pretends).
Unbounded watch loops are refused by this amendment: a seat that polls forever is a busy-wait
